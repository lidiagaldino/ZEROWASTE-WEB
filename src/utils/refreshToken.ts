export const refresh = async (email: string, senha: string) => {
    const url = 'https://webappdeploy-backend.azurewebsites.net/user'
    const options = {
        method: 'POST',
        body: JSON.stringify({
            email,
    ...
    localStorage.setItem('email', user.user.email)
      ...
      localStorage.setItem('tipo_pessoa', user.user.pessoa_juridica[0] > 0 ? 'Pessoa Juridica' : 'Pessoa Fisica')
